...
set output "convergence_full.pdf"
set logscale x
set logscale y; set ytics (0.8,0.95,1,1.05,""1.2,""1.4,""1.6,""1.8,2)
set yrange [0.8:2]
set bars small

set ylabel 'Ratio to {/Symbol \e}=e-9 for full'
set xlabel 'x'
...
plot res_5 i 9 using (exp(-$2)):(sqrt(exp(-$2))*$4)/(sqrt(exp(-$2))*$9):(sqrt(exp(-$2))*$5)/(sqrt(exp(-$2))*$9) with yerr ps 0 title '{/Symbol \e}=e-5',\
res_6 i 9 using (exp(-$2)):(sqrt(exp(-$2))*$4)/(sqrt(exp(-$2))*$9):(sqrt(exp(-$2))*$5)/(sqrt(exp(-$2))*$9) with yerr ps 0 title '{/Symbol \e}=e-6',\
res_7 i 9 using (exp(-$2)):(sqrt(exp(-$2))*$4)/(sqrt(exp(-$2))*$9):(sqrt(exp(-$2))*$5)/(sqrt(exp(-$2))*$9) with yerr ps 0 title '{/Symbol \e}=e-7',\
res_8 i 9 using (exp(-$2)):(sqrt(exp(-$2))*$4)/(sqrt(exp(-$2))*$9):(sqrt(exp(-$2))*$5)/(sqrt(exp(-$2))*$9) with yerr ps 0 title '{/Symbol \e}=e-8'
