...
combined i 0 using (exp(-$2)):(sqrt(exp(-$2))*$4)/(sqrt(exp(-$2))*$9):(sqrt(exp(-$2))*$5)/(sqrt(exp(-$2))*$9) t 'simple {/Symbol \t}=0.1' w yerr lc rgb"red" lw 3 pt 7 ps 0 lt 1
...
plot 1.0 lc rgb"black" lt 0 lw 3 notitle,\
combined i 9 using (exp(-$2)):(sqrt(exp(-$2))*$4)/(sqrt(exp(-$2))*$9):(sqrt(exp(-$2))*$5)/(sqrt(exp(-$2))*$9) t 'simple {/Symbol \t}=1' w yerr lc rgb"red" lw 3 pt 7 ps 0 lt 1
