...
res i 0 using (exp(-$2)):(sqrt(exp(-$2))*$4) with lines lc rgb"red" dt 3 lw 3  title 'full {/Symbol \t}=0.1',\
exact i 0 using (exp(-$2)):(sqrt(exp(-$2))*$4) with lines lt 1 lc rgb"red" title 'exact {/Symbol \t}=0.1',\
res_simple i 4 using (exp(-$2)):(sqrt(exp(-$2))*$4) with lines lc rgb"blue" dt 2 lw 3  title 'simple {/Symbol \t}=0.4',\
res i 4 using (exp(-$2)):(sqrt(exp(-$2))*$4):(sqrt(exp(-$2))*$5) with lines lc rgb"blue" dt 3 lw 3   title 'full {/Symbol \t}=0.4',\
exact i 4 using (exp(-$2)):(sqrt(exp(-$2))*$4) with lines lt 1 lc rgb"blue" title 'exact {/Symbol \t}=0.4',\
res_simple i 9 using (exp(-$2)):(sqrt(exp(-$2))*$4):(sqrt(exp(-$2))*$5) with lines lc rgb"#008000" dt 2 lw 3  title 'simple {/Symbol \t}=1',\
res i 9 using (exp(-$2)):(sqrt(exp(-$2))*$4)with lines lc rgb"#008000" dt 3 lw 3   title 'full {/Symbol \t}=1',\
